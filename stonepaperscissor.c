...
main()
{
    ...
    char you,computer,result;           //you denotes user's input and computer denotes guess of computer.Result will contain the final result of the game.

    srand(time(0));                     //srand function will choose random number every time.
    n=rand()%100;                       //to make the random number between 1 and 100, it is divided by 100

    if(n<33)                            //using simple probability 100 is roughly divided among stone,paper and scissor
        computer='s';                   //s is for stone

    else if(n>33 && n<66)
        computer='p';                   //p is for paper

    else
        computer='z';                   //z is for scissor
